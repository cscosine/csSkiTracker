...
namespace csSkiTracker {
  ...
  class DataReader {
    ...
    static bool readCalibSection(CalibPoints& cp, const QString& header, QFile& file, int& lineCount, QString& error, const Eigen::Matrix3Xd& allWorldPoints);

    template<class T>
    static bool readMatrix(Eigen::Ref<Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic>>  dest, QFile& file, int& lineCount, QString& error);
