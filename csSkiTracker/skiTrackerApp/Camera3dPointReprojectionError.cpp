...
#ifndef NDEBUG
#  define CAST_TYPE dynamic_cast
#else
#  define CAST_TYPE static_cast
#endif
  const CameraNode& camera = CAST_TYPE<const CameraNode&>(this->parameter(0));
  const PoseNode& pose = CAST_TYPE<const PoseNode&>(this->parameter(1));

  const PointNode* point3d = nullptr;
  const PointXYNode* pointXY = nullptr;
  int pointSize;
  Eigen::Vector3d point3dCoord;
  if (this->parameter(2).type() == CameraPosePointNodeBaseType::Point) {
    point3d = &CAST_TYPE<const PointNode&>(this->parameter(2));
    point3dCoord = point3d->point();
    pointSize = 3;
  } else if (this->parameter(2).type() == CameraPosePointNodeBaseType::PointXY) {
    pointXY = &CAST_TYPE<const PointXYNode&>(this->parameter(2));
    point3dCoord = pointXY->T_W_planeOrigin() * pointXY->point3d();
    pointSize = 2;
  } else
    assert(false);

  if (!hessians) {
    Eigen::Vector3d cameraPoints = pose.pose() * point3dCoord;
    Eigen::Vector2d pz1 = csCamera::Utils::points3D_to_z1(cameraPoints);
    Eigen::Vector2d pz1d = camera.distModel().distort(pz1);
    Eigen::Vector2d repr = camera.camera().pointsZ1_to_image(pz1d);
    Eigen::Vector2d err = repr - _imgMeasPoint;
    ...
    this->H_camParams_point.setZero(camera.numParams(), pointSize);

    this->H_pose.setZero(6, 6);
    this->H_pose_point.setZero(6, pointSize);

    this->H_point.setZero(pointSize, pointSize);

    this->b_camParams.setZero(camera.numParams());
    this->b_pose.setZero(6);
    this->b_point.setZero(pointSize);

    Eigen::Vector3d pcam = pose.pose() * point3dCoord;
    Eigen::Matrix3d d_pcam_d_point = pose.pose().linear();

    // local points
    Eigen::Matrix<double, 3, 6> d_pcam_d_g0 = csLie::se3JacobianLeftZeroPerturbationPointTransformation(pcam);
    ...
    Eigen::Matrix<double, 2, 3> d_repr_d_point = d_repr_d_pz1d * d_pz1d_d_pz1 * d_pz1_d_pcam * d_pcam_d_point;
    ...
    const double chi2 = err.squaredNorm();
    ...
    if (pointSize == 3) {
      ...
      this->H_point += d_repr_d_point.transpose() * d_repr_d_point;
      this->b_point += d_repr_d_point.transpose() * err;

      this->H_camParams_point += d_repr_d_camParams.transpose() * d_repr_d_point;
      this->H_pose_point += d_repr_d_g0.transpose() * d_repr_d_point;
    } else {
      Eigen::Matrix<double, 3, 2> d_p3d_d_pxy;
      d_p3d_d_pxy.setZero();
      d_p3d_d_pxy(0, 0) = 1;
      d_p3d_d_pxy(1, 1) = 1;
      Eigen::Matrix<double, 3, 2> d_p3dT_d_pxy = pointXY->T_W_planeOrigin().linear() * d_p3d_d_pxy;

      Eigen::Matrix<double, 2, 2> d_repr_d_pxy = d_repr_d_point * d_p3dT_d_pxy;

      this->H_point += d_repr_d_pxy.transpose() * d_repr_d_pxy;
      this->b_point += d_repr_d_pxy.transpose() * err;

      this->H_camParams_point += d_repr_d_camParams.transpose() * d_repr_d_pxy;
      this->H_pose_point += d_repr_d_g0.transpose() * d_repr_d_pxy;
    }
