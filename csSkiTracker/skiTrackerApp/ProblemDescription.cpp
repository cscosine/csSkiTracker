...
#define DEBUGME_CLOSEST if(false) std::cout
#define DEBUGME_VIEWRAYS if(false) std::cout
#define DEBUGME if(true) std::cout <<
...
Eigen::Matrix3Xd Problem::computeClosest3DPoint(const Eigen::Isometry3d& T_W_wrt_C, const Eigen::Matrix3Xd& wp, const Eigen::Matrix3Xd& unitView_C) {
...
void Problem::initView1(const Eigen::Matrix2Xd& imgPoints, const Eigen::ArrayXi& worldPointIndexes, Eigen::Vector2i imgSize) {
  ...
  this->_view1CalibPoints.reprojPoints = this->_view1CamPar.cameraModel.points3D_to_image(this->_T_W_wrt_view1 * worldPoints, this->_view1CamPar.distModel);
  this->_view1CalibPoints.p3d_wrt_cam_closest = computeClosest3DPoint(_T_W_wrt_view1, worldPoints, _view1CalibPoints.viewRaysUnit(_view1CamPar));
...
void Problem::initView2(int i, const Eigen::Matrix2Xd& imgPoints, const Eigen::ArrayXi& worldPointIndexes, Eigen::Vector2i imgSize) {
  ...
  if (imgPoints.cols() >= 6) {
    ...
    ProjectionMatrixEstimate::estimateCameraAndRt(imgPoints, worldPoints, this->_view2CamPar[i].cameraModel, this->_T_W_wrt_view2[i].second);
    _validViewsIndexes.push_back(i);

    this->_view2CalibPoints[i].reprojPoints = this->_view2CamPar[i].cameraModel.points3D_to_image(this->_T_W_wrt_view2[i].second * worldPoints, this->_view2CamPar[i].distModel);
    this->_view2CalibPoints[i].p3d_wrt_cam_closest = computeClosest3DPoint(this->_T_W_wrt_view2[i].second, worldPoints, _view2CalibPoints[i].viewRaysUnit(_view2CamPar[i]));
...
void Problem::setImgMeasPoints(int i, const Eigen::Matrix2Xd& view1_meas, const Eigen::Matrix2Xd& view2_meas, const Eigen::VectorXi& indexes) {
  ...
  this->_framesMeas[i].view1Points_repr = this->_view1CamPar.cameraModel.points3D_to_image(this->_T_W_wrt_view1 * this->_framesMeas[i].p3d_est);
  this->_framesMeas[i].view2Points_repr = this->_view2CamPar[i].cameraModel.points3D_to_image(this->_T_W_wrt_view2[i].second * this->_framesMeas[i].p3d_est);
}
...
void Problem::recomputeImgMeasTriangulation() {
  for (int i = 0; i < this->_framesMeas.size(); i++) {
    ...
    this->_framesMeas[i].view1Points_repr = this->_view1CamPar.cameraModel.points3D_to_image(this->_T_W_wrt_view1 * this->_framesMeas[i].p3d_est);
    this->_framesMeas[i].view2Points_repr = this->view2CamPar(i).cameraModel.points3D_to_image(this->_T_W_wrt_view2[i].second * this->_framesMeas[i].p3d_est);
  }
}
...
Eigen::Vector2d Problem::view2fxfyMedian() const {
  Eigen::Vector2d ret(
    median(this->collectView2Fx()),
    median(this->collectView2Fy())
  );
  return ret;
}
...
Eigen::Vector2d Problem::view2cxcyMedian() const {
  Eigen::Vector2d ret(
    median(this->collectView2Cx()),
    median(this->collectView2Cy())
  );
  return ret;
}
...
void Problem::recomputeCameraParamsPoses() {
  ...
  CameraNode camera1(
    this->view1CamPar().cameraModel,
    this->view1CamPar().distModel,
    CameraNode::FocalEstimation::Both,
    { false, false }, //center
  ...
  CameraNode camera2(
    this->uniqueCamParamsView2() ? 
      ...
      csCamera::Camerad(
        ...
        view2cxcyMedian.x(),
        view2cxcyMedian.y(),
        this->view2CamPar(0).cameraModel.w(),
        this->view2CamPar(0).cameraModel.h()
      ),
    this->uniqueCamParamsView2() ?
      this->view2CamPar(0).distModel :
      csCamera::CameraDistortionModeld(
        0, 0, 0,   // k123
        0, 0,      // p12
        0, 0, 0,   // k456
        0, 0, 0, 0 // s1234
  ...
  section.addEdge(
    ...
    new Camera3dPointReprojectionError(recoverPoints(this->view1CalibPoints().indexes, _calibWorldPoints), this->view1CalibPoints().imgPoints)
  );
    ...
    for (int i = 0; i < this->numViews(); i++) {
      if (this->T_W_wrt_view2(i).first) {
        section.addEdge(
          ...
          new Camera3dPointReprojectionError(recoverPoints(this->view2CalibPoints(i).indexes, _calibWorldPoints), this->view2CalibPoints(i).imgPoints)
        );
      }
    }
  }
  ...
  csNelson::LevenbergMarquardt<typename csNelson::SolverTraits<csNelson::solverCholeskyDense>::Solver<typename CameraCalibSection::Hessian::Traits>>  lm;
  ...
  if (section.numView2Poses() > 0) {
    for (int i = 0; i < this->numViews(); i++) {
      if (this->T_W_wrt_view2(i).first) {
        ...
        this->_view2CalibPoints[i].reprojPoints = this->_view2CamPar[0].cameraModel.points3D_to_image(this->_T_W_wrt_view2[i].second * worldPoints, this->_view2CamPar[0].distModel);
        this->_view2CalibPoints[i].p3d_wrt_cam_closest = computeClosest3DPoint(this->_T_W_wrt_view2[i].second, worldPoints, _view2CalibPoints[i].viewRaysUnit(_view2CamPar[0]));
