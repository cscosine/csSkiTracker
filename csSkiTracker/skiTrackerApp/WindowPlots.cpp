...
/*
...

  {
    ...
    // recompute 3d points
    Eigen::VectorXd error3d = (problem.T_W_wrt_view1().inverse() * problem.view1CalibPoints().p3d_wrt_cam_closest - problem.view1CalibPoints3D()).colwise().norm();
...
/*
...

  {
    ...
    // recompute 3d points
    Eigen::VectorXd error3d = (problem.T_W_wrt_view2(view2Num).second.inverse() * problem.view2CalibPoints(view2Num).p3d_wrt_cam_closest - problem.view2CalibPoints3D(view2Num)).colwise().norm();
    std::vector<double> e3d(error3d.size());
    for (int i = 0; i < e3d.size(); i++) {
      e3d[i] = error3d(i);
    }
...
void Window::on_pushButtonPlotCameraParams_clicked() {
  ...
  plt::named_plot("Fixed Camera", std::vector<double>{problem.view1CamPar().cameraModel.cx()}, std::vector<double>{problem.view1CamPar().cameraModel.cy()}, "r+");
  plt::named_plot("Moving Camera", view2cx, view2cy, "g+");
  for (int i = 0; i < view2cx.size(); i++) {
    plt::text(view2cx[i], view2cy[i], QString("%1").arg(view2idx[i]).toStdString());
  }
...
/*
  ...
  }
  {
    ...
    {
      Eigen::VectorXd dist = (problem.T_W_wrt_view1().inverse() * problem.view1CalibPoints().p3d_wrt_cam_closest - problem.view1CalibPoints3D()).colwise().norm();
      e_mean[0] = dist.mean();
      e_errMax[0] = (dist.array() - e_mean[0]).cwiseAbs().maxCoeff();
    }
    for (int i = 0; i < idx.size(); i++) {
      Eigen::VectorXd dist = (problem.T_W_wrt_view2(i).second.inverse() * problem.view2CalibPoints(i).p3d_wrt_cam_closest - problem.view2CalibPoints3D(i)).colwise().norm();
      e_mean[i+1] = dist.mean();
      e_errMax[i+1] = (dist.array() - e_mean[i+1]).cwiseAbs().maxCoeff();
    }
...
void Window::saveAndShow() {
  /*
  QString filename = QDir::temp().absoluteFilePath(QString("%1-%2-plot.svg").arg(QCoreApplication::applicationName()).arg(QCoreApplication::applicationPid()));
